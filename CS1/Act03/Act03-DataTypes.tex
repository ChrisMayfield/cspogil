...
During report-out, introduce the term \emph{literal} and explain that \texttt{float} is a floating-point data type.
Ask students to guess where the name \texttt{double} comes from.
...
On \ref{twostrs}, have 2--3 teams draw their diagrams on the board and address misconceptions.
It might be helpful at this point to direct students to \href{https://cscircles.cemc.uwaterloo.ca/java_visualize/}{Java Visualizer}, which draws similar diagrams (including stack frames).
Be sure to check the option for ``Show String/Integer/etc objects, not just values'' so it will draw references as arrows.
